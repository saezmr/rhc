...
public class CSVTool {
	...
	System.out.printf("FIT CSV Tool %d.%d.%d.%d\n", Fit.PROTOCOL_VERSION_MAJOR,
		Fit.PROTOCOL_VERSION_MINOR, Fit.PROFILE_VERSION_MAJOR, Fit.PROFILE_VERSION_MINOR);
	args = new String[] { "-b", "/home/rsaez/Documents/GarminActivities/fit/2013-11-30-09-00-51.fit",
		"/home/rsaez/Documents/GarminActivities/fit/2013-11-30-09-00-51.csv" };
	while (arg < args.length) {
		...
		if (nextArgumentDefinition > 0) {
		    mesgDefinitionsToOutput = new ArrayList<String>(Arrays.asList(args[arg].toLowerCase()
			    .split(",")));
		...
		if ((nextArgumentData == 0) && (dataMessagesToOutput.isEmpty())) {
		    System.out
			    .println("No data messages defined for --data option.  Use 'none' if no data is desired.");
		    return;
		}
	    }
	    arg++;
	...
	if (fitToCsv) {
		...
		out = out.substring(0, out.length() - 4); // Remove .csv
							  // extension.
		...
		// MesgDataCSVWriter dataMesgWriter = new MesgDataCSVWriter(out
		// + "_data.csv");
