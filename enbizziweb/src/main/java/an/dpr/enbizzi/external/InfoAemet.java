package an.dpr.enbizzi.external;
...
import an.dpr.enbizzi.beans.AemetEstadoCielo;
import an.dpr.enbizzi.beans.AemetHora;
import an.dpr.enbizzi.beans.AemetLocalidadTags;
import an.dpr.enbizzi.beans.AemetPeriodo;
import an.dpr.enbizzi.beans.AemetViento;
import an.dpr.enbizzi.beans.Orache;
import an.dpr.enbizzi.beans.PrediccionAemet;
...
public class InfoAemet {
	...
	if (oracheEnFechas(salida)) {
	    try {
		PrediccionAemet pa;
		pa = getPrediccion(salida.getAemetStart(), salida.getDate());
		if (pa != null) {
		    Orache oStart = Orache.getOrache(pa);
		    Orache oStop = null;
		    if (salida.getAemetStop() != null) {
			pa = getPrediccion(salida.getAemetStop(),
				salida.getDate());
			oStop = Orache.getOrache(pa);
		    }
		    orache = new Orache[] { oStart, oStop };
		}
    ...
    public PrediccionAemet getPrediccion(Integer aemetCode, Date fecha)
    ...
    private String leerXML(Integer aemetCode) throws IOException {
    ...
    private String getAemetUrl(Integer aemetCode) {
